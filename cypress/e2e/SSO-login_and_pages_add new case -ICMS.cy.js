	...
	it('_main', function () {
		for (let i = 0; i < 20; i++) {
			...
			cy.get('@user_data').each((user) => {
				...
				cy.get('#FirstName').type('BinquanAutomationTesting');
				//cy.get('#LastName').click();
				cy.get('#LastName').type('Wang');
				//cy.get('#City').click();
				cy.get('#City').type('City1');
				//cy.get('#StateId_listbox').select('MD');
				//cy.get('#StateId_listbox').select('MD'); //debug
				cy.get('[aria-owns="StateId_listbox"]').type('NY');
				...
				//cy.get('#Email').click();
				cy.get('#Email').type('binquan.wang@dsfederal.com');
			    ...
			    //cy.get('#EmployerTypeId_listbox').click();
				//cy.get('#EmployerTypeId_listbox').select(0).should('have.value', 'Department of Defense (DoD)');//debug
				//cy.get('#EmployerTypeId_listbox').select('Local Goverment');//debug
				cy.get('[aria-owns="EmployerTypeId_listbox"]').click();
				cy.get('[aria-owns="EmployerTypeId_listbox"]').type('Department of Defense (DoD)');
				//cy.get('#EmployerCity').click();
				cy.get('#EmployerCity').type('city2');
				cy.get('[aria-owns="EmployerStateId_listbox"]').type('MD');
				//cy.get('#EmployerStateId').select('MD');
				//cy.get('#PriPocFirstName').click();
				cy.get('#PriPocFirstName').type('firstnn');
				//cy.get('#PriPocLastName').click();
				cy.get('#PriPocLastName').type('lastnn');
				//cy.get('#PriPocFirstName').click();
				...
				cy.get('[aria-owns="HowCaseReceivedId_listbox"]').type('Phone');
				cy.get('#Terminated_False').click();
				cy.get('#PrivacyActDisclosed_True').click();
				cy.get('#InActiveDuty_True').click();
				cy.get('[aria-owns="ProblemCodeId_listbox"]').type('Other');
